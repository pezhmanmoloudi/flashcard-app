...
export function useCardSwipe(options: UseCardSwipeOptions = {}) {
  ...
  const isSwipingRight = computed(() => dragX.value > threshold * 0.4)
  const isSwipingLeft = computed(() => dragX.value < -threshold * 0.4)
  ...
  function onPointerUp() {
    ...
    if (Math.abs(dx) >= threshold || Math.abs(velocityX) > 12) {
      ...
      setTimeout(() => {
        // Disable transition before resetting so the new card snaps to center
